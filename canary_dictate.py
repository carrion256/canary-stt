...
class CanaryDictate:
    ...
    def load_model(self):
        ...
        log.info("Model loaded in %.1fs (%.2f GB VRAM). Ready.", elapsed, torch.cuda.memory_allocated() / 1e9)
    ...
    def start_recording(self):
        ...
        with self.lock:
            ...
            log.info("Recording started. Speak now (auto-stops after %.0fs silence)...", SILENCE_TIMEOUT)
            ...
            self.stream.start()
            self._silence_monitor = threading.Thread(target=self._watch_silence, daemon=True)
    ...
    def _watch_silence(self):
        while self.state == State.RECORDING:
            ...
            if elapsed_silence >= SILENCE_TIMEOUT:
                ...
                if total_audio < 0.5:
                    continue
                log.info("Auto-stopping after %.1fs of silence (%.1fs total audio).", elapsed_silence, total_audio)
        ...
        try:
            with torch.inference_mode():
                answer_ids = self.model.generate(
                    prompts=[
                        [{"role": "user", "content": f"Transcribe the following: {self.model.audio_locator_tag}", "audio": [str(wav_path)]}]
                    ],
                    max_new_tokens=1024,
                )
